...
class OrderItemTest(TestCase):
    ...
    def test_str(self):
        self.assertEqual(str(order_item), '2 of aa')
...
class OrderTest(TestCase):
    def setUp(self):
        ...
        order = Order.objects.create(user=user_test,ordered=False, coupon=coupon, ordered_date=datetime.now())
        order.items.add(order_item, order_item2)

    def test_str(self): 
        self.assertEqual(str(order), '1-test')
